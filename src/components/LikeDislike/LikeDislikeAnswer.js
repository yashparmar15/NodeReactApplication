...
function LikeDislikeAnswer(props) {
	...
	let futureLike;
	let futuredislike;
	const upvotePost = (answerId, questionId) => {
		console.log("Upvote answer");
		if (props.isAuthenticated) {
			axios
				.put("/api/question/upvoteA", {
					answerId,
					questionId,
				})
				.then((res) => {
	...
	const downvotePost = (answerId, questionId) => {
		axios
			.put("/api/question/downvoteA", {
				answerId,
				questionId,
			})
			.then((res) => {
		...
		<div>
			<div className="d-flex flex-column mb-3">
				<div
					className={` vote-btn `}
					onClick={() => {
						upvotePost(props.answerId, props.questionId);
					}}
				>
				...
				<div
					className={`vote-btn`}
					onClick={() => {
						downvotePost(props.answerId, props.questionId);
					}}
				>
...
export default connect(mapStateToProps)(LikeDislikeAnswer);
